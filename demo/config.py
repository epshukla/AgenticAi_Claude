...
# Get the absolute path to this demo directory (where agent.py lives)
DEMO_DIR = Path(__file__).parent.absolute()

# Target project directory - COMPLETELY SEPARATE from demo
# Uses relative path from parent directory (portable across machines)
# Assumes structure: CyberCypher26/ecommerce-website/
...
ALLOWED_EXTENSIONS = {".md", ".txt", ".json", ".yaml", ".yml", ".py", ".config", ".html", ".css", ".js", ".jsx"}
...
# API key is read from environment variable (not hardcoded for security)
# The agent will use this to communicate with Claude
CLAUDE_API_KEY = os.environ.get("CLAUDE_API_KEY", "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")
CLAUDE_MODEL = "claude-opus-4-1-20250805"  # Latest Claude model
